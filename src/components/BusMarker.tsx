...
const getBusIcon = (heading: number, isSelected?: boolean) => new L.DivIcon({
  className: 'bg-transparent',
  html: `
    <div class="relative w-8 h-8 flex items-center justify-center">
      ${isSelected ? '<div class="absolute w-12 h-12 bg-[#FFCC00]/50 rounded-full animate-pulse z-0"></div>' : ''}
      <div class="absolute w-8 h-8 bg-carris-yellow/40 rounded-full animate-ping z-0"></div>
      <div 
        class="absolute w-6 h-6 border-2 rounded-full flex items-center justify-center shadow-lg pointer-events-none z-20 ${isSelected ? 'border-white bg-[#FFCC00]' : 'border-carris-dark bg-carris-light'}"
      >
      ...
      <div 
        ...
        style="transform: rotate(${heading}deg); border-top: 4px solid ${isSelected ? '#FFFFFF' : '#FFCC00'}; border-right: 4px solid transparent; border-left: 4px solid transparent; border-radius: 50%;"
      ></div>
    </div>
  `,
  iconSize: [32, 32],
...
const BusMarker = memo(function BusMarker({ vehicle, isSelected }: BusMarkerProps) {
  ...
  return (
    <Marker 
      position={[lat, lon]} 
      icon={getBusIcon(vehicle.bearing || vehicle.heading || 0, isSelected)}
